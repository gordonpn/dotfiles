...
### What It Does
1. **Installs MCP Binaries:** Downloads and verifies `github-mcp-server` into `~/.local/bin`, and ensures `terraform-mcp-server` is installed and linked (via Homebrew or `go install`).
